#define _USE_MATH_DEFINES
#include<math.h>
#include<stdio.h>
#include<string.h>
#include <stdlib.h>

extern "C" {
#include"./SDL2-2.0.10/include/SDL.h"
#include"./SDL2-2.0.10/include/SDL_main.h"
}

//=================================
//    INICJALIZACJA STA£YCH
//=================================

// ekran
#define SCREEN_WIDTH 640
#define SCREEN_HEIGHT 480
#define HORIZON_Y 200
#define CAMERA_MARGIN 100
#define FPS_REFRESH 0.5
#define ANIM_PATH "./textures/animations/"
#define DEFAULT_FONT_SIZE 8
// player
#define PLAYER_START_X_POS 45
#define PLAYER_SPEED 350.0 // pixele na sekundê
#define DASH_SPEED 700.0
#define PLAYER_WIDTH 128
#define PLAYER_HEIGHT 128
#define HURTBOX_WIDTH 40
#define HURTBOX_HEIGHT 90
#define HURTBOX_OFF_X 44
#define HURTBOX_OFF_Y 44
#define HIT_X_OFFSET 15
// enemy
#define ENEMY_INIT_WIDTH 128
#define ENEMY_INIT_HEIGHT 128
#define ENEMY_POS_X 500
#define ENEMY_POS_Y 350
// enemies
#define MAX_ENEMIES 2
#define ENEMY_TYPE_SIMPLE 0
#define ENEMY_TYPE_CHARGER 1
#define ENEMY_SPEED_WALK 70.0
#define ENEMY_ATTACK_COOLDOWN 3.0
// enemy simple
#define ENEMY_SIMPLE_ATTACK_RANGE 60
#define ENEMY_SIMPLE_PREPARE_TIME 0.5
// enemy charger
#define ENEMY_SPEED_CHARGE 300.0
#define CHARGER_KEEP_DIST 200
#define TIME_CHARGER_PREPARE 1.2
#define TIME_CHARGE 3.0
#define CHARGER_ALIGN_Y_MARGIN 10
#define AI_STATE_ALIGN 0
#define AI_STATE_PREPARE 1
#define AI_STATE_CHARGE 2
#define AI_STATE_COOLDOWN 3
// predkoœci akcji
#define ANIM_SPEED_WALK 0.1
#define ANIM_SPEED_IDLE 0.35
#define TIME_ATTACK_LIGHT 0.3
#define TIME_ATTACK_HEAVY 0.8
#define TIME_JUMP 1.0
#define TIME_DASH 0.3
#define TIME_HURT 1
#define TIME_ATTACK_SHOULDER 0.8
// combo
#define COMBO_COOLDOWN 2.0
#define COMBO_MULTIPLER_ADD_VALUE 0.4
#define COMBO_MAX_SCALE 8
#define COMBO_EFFECT_X_OFFSET 20
#define COMBO_TRESHOLD_1 2.0
#define COMBO_TRESHOLD_2 3.0
#define COMBO_TRESHOLD_3 4.0
#define COMBO_TRESHOLD_4 5.0
#define COMBO_Y_OFFSET 80
#define COMBO_VIBRATION_DEVIATION 10
#define COMBO_DEVIATION_Y_OFFSET 5
#define MAX_MULTIPLER_VALUE 5
// directions
#define LEFT 1
#define RIGHT 0
// actions
#define ACTION_IDLE 0
#define ACTION_LIGHT 1
#define ACTION_HEAVY 2
#define ACTION_JUMP 3
#define ACTION_WALK 4
#define ACTION_DASH 5
#define ACTION_HURT 6
#define ACTION_SHOULDER 7
#define ACTION_CHARGE 8
#define ACTIONS_COUNT 8
// buffer
#define INPUT_BUFFER_SIZE 5
#define MAX_SEQ_LEN 10
#define MAX_SEQUENCES 5
// info
#define INFO_MARGIN 4
#define INFO_BOX_HEIGHT 54
#define INFO_TEXT_BUFFER_SIZE 128
#define CHAR_WIDTH 8
#define INFO_LINE_1_Y 10
#define INFO_LINE_2_Y 26
#define INFO_LINE_3_Y 42

enum GameInput {
	INPUT_NONE = 0,
	INPUT_UP,
	INPUT_DOWN,
	INPUT_LEFT,
	INPUT_RIGHT,
	INPUT_ATTACK_LIGHT,
	INPUT_ATTACK_HEAVY,
	INPUT_DEV_MODE
};

//=================================
//       STRUKTURY
//=================================

struct AttackData {
	int width;
	int height;
	int yOffset;
	int damage;
	bool active;
};

struct Sequence {
	const char* name;
	int sequence[MAX_SEQ_LEN];
	int len;
	int maxTimeGap;
};

struct InputEvent {
	int input;
	Uint32 time;
};

struct EntityTextures {
	SDL_Texture* idle;
	int idleFrames;
	SDL_Texture* attLight;
	int attLightFrames;
	SDL_Texture* attHeavy;
	int attHeavyFrames;
	SDL_Texture* jump;
	int jumpFrames;
	SDL_Texture* dash;
	int dashFrames;
	SDL_Texture* walk;
	int walkFrames;
	SDL_Texture* hurt;
	int hurtFrames;
	SDL_Texture* attShoulder;
	int attShoulderFrames;
};

struct Entity {
	double x, y;
	int w, h;
	int direction;
	int currentAction;
	double actionTimer;
	bool hasHit;
	double hitTimer;
	EntityTextures textures;
	int currentFrame;
	double animTimer;
};

struct Player: Entity {
	double speed;
};

struct Enemy: Entity {
	bool active;
	int type;
	double attackCooldown;
	int aiState;
	double aiTimer;
};

struct Colors {
	int white;
	int black;
	int green;
	int red;
	int blue;
	int lightBlue;
	int lightGreen;
	int combo1;
	int combo2;
	int combo3;
	int combo4;
	int combo5;
};

struct Camera {
	int x;
	int y;
	int w;
	int h;
};

struct Time {
	double worldTime;
	double delta;
	int t1;
	int t2;
	int frames;
	double fpsTimer;
	double fps;
};

struct Buffer {
	InputEvent inputs[INPUT_BUFFER_SIZE];
	int headIndex;
	int previousHeadIndex;
	bool showDebug;
};

struct Score {
	int points;
	double comboMultipler;
	double comboTimer;
};

// g³ówna struktura stanu gry
struct GameState {
	Player player;
	Enemy enemies[MAX_ENEMIES];
	Colors colors;
	Camera camera;
	Time time;
	Buffer buffer;
	Score score;
	int quit;

	Sequence definedSequences[MAX_SEQUENCES];
	int sequencesCount;
};

// struktura obs³uguj¹ca bibliotekê SDL
struct SDLContext {
	SDL_Window* window;
	SDL_Renderer* renderer;
	SDL_Surface* screen;
	SDL_Texture* scrtex;
	SDL_Surface* charset;

	SDL_Texture* background;
	int bgWidth;
	int bgHeight;
};

// tablice nazw wprowadzonych akcji (do opcji debug)
const char* actionNames[] = {
	"IDLE", // 0
	"LIGHT", // 1
	"HEAVY", // 2
	"JUMP", // 3
	"WALK", // 4
	"DASH", // 5
	"HURT", // 6
	"SHOULDER", // 7
	"CHARGE", // 8
};
const char* inputNames[] = {
	"...",	// INPUT_NONE
	"UP",	// INPUT_UP
	"DOWN",	// INPUT_DOWN
	"LEFT",	// INPUT_LEFT
	"RIGHT",// INPUT_RIGHT
	"L_ATK",// INPUT_ATTACK_LIGHT
	"H_ATK",// INPUT_ATTACK_HEAVY
	"DEV"	// INPUT_DEV_MODE
};

// dane do hitboxów ataków
// (w, h, yOffset, damage, active)
const AttackData attacksData[] = {
	{ 0, 0, 0, 0, false },		// 0 ACTION_IDLE
	{ 40, 20, 60, 10, true },	// 1 ACTION_LIGHT
	{ 50, 30, 60, 20, true },	// 2 ACTION_HEAVY
	{ 40, 40, 100, 15, true },	// 3 ACTION_JUMP
	{ 0, 0, 0, 0, false },		// 4 ACTION_WALK
	{ 0, 0, 0, 0, false },		// 5 ACTION_DASH
	{ 0, 0, 0, 0, false },		// 6 ACTION_HURT
	{ 20, 80, 80, 25, true },	// 7 ACTION_SHOULDER
	{ 10, 80, 80, 10, true }	// 7 ACTION_CHARGE
};

//=================================
//       FUNKCJE POMOCNICZE
//=================================

// funkcja ³aduj¹ca dan¹ teksturê z pliku BMP pobieraj¹ca œcie¿kê i ustawiaj¹ca szerokoœæ i wysokoœæ
// pod podane adresy (outW, outH). Zwraca wskaŸnik do tekstury lub NULL w przypadku b³êdu.
SDL_Texture* loadTexture(SDL_Renderer* renderer, const char* path, int* outW, int* outH) {
	SDL_Surface* surface = SDL_LoadBMP(path);
	if (!surface) {
		printf("SDL_LoadBMP(%s) error: %s\n", path, SDL_GetError());
		return NULL;
	}

	if (outW) *outW = surface->w;
	if (outH) *outH = surface->h;

	SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, surface);
	SDL_FreeSurface(surface);

	if (!texture) {
		printf("SDL_CreateTextureFromSurface(%s) error: %s\n", path, SDL_GetError());
	}
	return texture;
}

// funkcja zwracaj¹ca prostok¹t hitboxa ataku w której siê znajduje dana postaæ
SDL_Rect getHitBox(Entity* entity) {
	SDL_Rect box = { 0, 0, 0, 0 };

	if (entity->currentAction < 0 || entity->currentAction > ACTIONS_COUNT) return box;

	AttackData data = attacksData[entity->currentAction];

	if (!data.active) return box;

	box.w = data.width;
	box.h = data.height;
	box.y = entity->y - data.yOffset;

	// ustawianie kierunku ataku
	if (entity->direction == RIGHT) {
		box.x = entity->x + entity->w - HURTBOX_OFF_X - HIT_X_OFFSET;
	}
	else {
		box.x = entity->x - box.w + HURTBOX_OFF_X + HIT_X_OFFSET;
	}

	return box;
}

// funkcja zwracaj¹ca prostok¹t hurtboxa danej postaci
SDL_Rect getHurtbox(Entity* entity) {
	SDL_Rect box;
	box.x = entity->x + HURTBOX_OFF_X;
	box.y = entity->y - entity->h + HURTBOX_OFF_Y;

	box.w = HURTBOX_WIDTH;
	box.h = HURTBOX_HEIGHT;

	return box;
}

// funkcja sprawdzaj¹ca czy dwa prostok¹ty nachodz¹ na siebie
bool checkRectCollision(SDL_Rect a, SDL_Rect b) {
	if (a.x + a.w < b.x) return false;
	if (a.x > b.x + b.w) return false;
	if (a.y + a.h < b.y) return false;
	if (a.y > b.y + b.h) return false;
	return true;
}

// funkcja zwracaj¹ca odpowiedni¹ teksturê gracza, odpowiedni¹ do akcji któr¹ w³aœnie wykonuje
SDL_Texture* getCurrentTexture(Entity* entity) {
	switch (entity->currentAction) {
	case ACTION_IDLE: return entity->textures.idle;
	case ACTION_LIGHT: return entity->textures.attLight;
	case ACTION_HEAVY: return entity->textures.attHeavy;
	case ACTION_JUMP: return entity->textures.jump;
	case ACTION_WALK: return entity->textures.walk;
	case ACTION_DASH: return entity->textures.dash;
	case ACTION_HURT: return entity->textures.hurt;
	case ACTION_SHOULDER: return entity->textures.attShoulder;
	case ACTION_CHARGE: return entity->textures.dash;
	default: return entity->textures.idle;
	}
}

// funkcja sprawdza element n-ty od koñca
// 0 - zwraca ostatni, 1 - przedostatni
InputEvent* getInputBack(Buffer* buffer, int stepsBack) {
	int index = (buffer->headIndex - 1 - stepsBack + INPUT_BUFFER_SIZE) % INPUT_BUFFER_SIZE;
	return &buffer->inputs[index];
}

// funkcja pomocnicza zwracaj¹ca czas trwania danej akcji
double getActionDuration(int action) {
	switch (action) {
	case ACTION_IDLE: return ANIM_SPEED_IDLE;
	case ACTION_WALK: return ANIM_SPEED_WALK;
	case ACTION_LIGHT: return TIME_ATTACK_LIGHT;
	case ACTION_HEAVY: return TIME_ATTACK_HEAVY;
	case ACTION_JUMP: return TIME_JUMP;
	case ACTION_DASH: return TIME_DASH;
	case ACTION_HURT: return TIME_HURT;
	case ACTION_SHOULDER: return TIME_ATTACK_SHOULDER;
	case ACTION_CHARGE: return TIME_DASH;
	default: return 0.1;
	}
}

// funckcja pomocnicza zwracaj¹ca liczbê klatek akcji w której znajduje siê dana postaæ
int getActionFrames(Entity* entity) {
	switch (entity->currentAction) {
	case ACTION_IDLE: return entity->textures.idleFrames;
	case ACTION_WALK: return entity->textures.walkFrames;
	case ACTION_LIGHT: return entity->textures.attLightFrames;
	case ACTION_HEAVY: return entity->textures.attHeavyFrames;
	case ACTION_JUMP: return entity->textures.jumpFrames;
	case ACTION_DASH: return entity->textures.dashFrames;
	case ACTION_HURT: return entity->textures.hurtFrames;
	case ACTION_SHOULDER: return entity->textures.attShoulderFrames;
	case ACTION_CHARGE: return entity->textures.dashFrames;
	default:            return 0;
	}
}

//=================================
//       FUNCKCJE SETUP
//=================================

// inicjalizacja gracza
void initPlayer(Player* player) {
	player->w = PLAYER_WIDTH;
	player->h = PLAYER_HEIGHT;
	player->x = SCREEN_WIDTH / 2;
	player->y = SCREEN_HEIGHT / 2;
	player->speed = PLAYER_SPEED;
	player->direction = RIGHT;
	player->currentAction = ACTION_IDLE;
	player->actionTimer = 0.0;
	player->hasHit = false;
	player->currentFrame = 0;
	player->animTimer = 0.0;
}

// ustawiania wroga, nadanie typu i pozycji startowej x, y
void spawnEnemy(Enemy* enemy, int type, double x, double y) {
	enemy->active = true;
	enemy->type = type;
	enemy->x = x;
	enemy->y = y;
	enemy->w = ENEMY_INIT_WIDTH;
	enemy->h = ENEMY_INIT_HEIGHT;
	enemy->currentAction = ACTION_IDLE;
	enemy->direction = LEFT;
	enemy->actionTimer = 0.0;
	enemy->hasHit = false;
	enemy->attackCooldown = ENEMY_ATTACK_COOLDOWN;
	enemy->hitTimer = 0.0;
	enemy->aiState = AI_STATE_ALIGN;
	enemy->aiTimer = 0.0;
	enemy->currentFrame = 0;
	enemy->animTimer = 0.0;
}

// inicjalizacja wrogów
void initEnemies(GameState* state) {
	spawnEnemy(&state->enemies[0], ENEMY_TYPE_SIMPLE, 500, 350);
	spawnEnemy(&state->enemies[1], ENEMY_TYPE_CHARGER, 700, 350);
}

// inicjalizacja pozycji i wielkoœci kamery
void initCamera(Camera* camera) {
	camera->x = 0;
	camera->y = 0;
	camera->w = SCREEN_WIDTH;
	camera->h = SCREEN_HEIGHT;
}

// inicjalizacja zmiennych czasu
void initTime(Time* time) {
	time->worldTime = 0;
	time->t1 = SDL_GetTicks();
	time->frames = 0;
	time->fpsTimer = 0;
	time->fps = 0;
}

// inicjalizacja kolorów
void initColors(Colors* colors, const SDL_PixelFormat* format) {
	colors->white = SDL_MapRGB(format, 0xFF, 0xFF, 0xFF);
	colors->black = SDL_MapRGB(format, 0x00, 0x00, 0x00);
	colors->green = SDL_MapRGB(format, 0x00, 0xFF, 0x00);
	colors->red = SDL_MapRGB(format, 0xFF, 0x00, 0x00);
	colors->blue = SDL_MapRGB(format, 0x11, 0x11, 0xCC);
	colors->lightBlue = SDL_MapRGB(format, 0x5D, 0xED, 0xF7);
	colors->lightGreen = SDL_MapRGB(format, 0x7E, 0xC8, 0x50);

	// kolory do combo napisu
	colors->combo1 = SDL_MapRGB(format, 255, 180, 180);
	colors->combo2 = SDL_MapRGB(format, 255, 80, 80);
	colors->combo3 = SDL_MapRGB(format, 255, 0, 0);
	colors->combo4 = SDL_MapRGB(format, 180, 0, 0);
	colors->combo5 = SDL_MapRGB(format, 100, 0, 0);
}

// inicjalizacja bufora wprowadzanych akcji
void initBuffer(Buffer* buffer) {
	buffer->headIndex = 0;
	buffer->previousHeadIndex = 0;
	buffer->showDebug = false;

	for (int i = 0; i < INPUT_BUFFER_SIZE; i++) {
		buffer->inputs[i].input = INPUT_NONE;
		buffer->inputs[i].time = 0;
	}
}

// inicjalizacja sekwencji combo
void initSequences(GameState* state) {
	state->sequencesCount = 0;

	Sequence* s1 = &state->definedSequences[state->sequencesCount++];
	s1->name = "Shoulder";
	s1->len = 3;
	s1->sequence[0] = INPUT_ATTACK_LIGHT;
	s1->sequence[1] = INPUT_ATTACK_LIGHT;
	s1->sequence[2] = INPUT_ATTACK_LIGHT;
	s1->maxTimeGap = 250;

	Sequence* s2 = &state->definedSequences[state->sequencesCount++];
	s2->name = "Dash Right";
	s2->len = 2;
	s2->sequence[0] = INPUT_RIGHT;
	s2->sequence[1] = INPUT_RIGHT;
	s2->maxTimeGap = 200;

	Sequence* s3 = &state->definedSequences[state->sequencesCount++];
	s3->name = "Dash Left";
	s3->len = 2;
	s3->sequence[0] = INPUT_LEFT;
	s3->sequence[1] = INPUT_LEFT;
	s3->maxTimeGap = 200;

	Sequence* s4 = &state->definedSequences[state->sequencesCount++];
	s4->name = "Jump";
	s4->len = 4;
	s4->sequence[0] = INPUT_ATTACK_HEAVY;
	s4->sequence[1] = INPUT_ATTACK_LIGHT;
	s4->sequence[2] = INPUT_ATTACK_HEAVY;
	s4->sequence[3] = INPUT_ATTACK_LIGHT;
	s4->maxTimeGap = 200;
}

// inicjalizacja zmiennych odnoœnie wyniku i combo
void initScore(Score* score) {
	score->points = 0;
	score->comboMultipler = 1;
	score->comboTimer = 0.0;
}

// inicjalizacja stanu gry oraz kolorów(dlatego przekazujemy surface ekranu)
void initGameState(GameState* state, const SDLContext* sdl) {
	initPlayer(&state->player);
	initEnemies(state);
	initCamera(&state->camera);
	initTime(&state->time);
	initColors(&state->colors, sdl->screen->format);
	initBuffer(&state->buffer);
	initSequences(state);
	initScore(&state->score);
	state->quit = 0;
}

// funkcja inicjalizuj¹ca bibliotekê SDL
bool initSDL(SDLContext* sdl) {
	if (SDL_Init(SDL_INIT_EVERYTHING) != 0) {
		printf("SDL_Init error: %s\n", SDL_GetError());
		return false;
	}

	// tryb pe³noekranowy / fullscreen mode
//	int rc = SDL_CreateWindowAndRenderer(0, 0, SDL_WINDOW_FULLSCREEN_DESKTOP,
//	                                 &window, &renderer);
	int rc = SDL_CreateWindowAndRenderer(SCREEN_WIDTH, SCREEN_HEIGHT, 0,
		&sdl->window, &sdl->renderer);
	if (rc != 0) {
		SDL_Quit();
		printf("SDL_CreateWindowAndRenderer error: %s\n", SDL_GetError());
		return 1;
	};

	SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "linear");
	SDL_RenderSetLogicalSize(sdl->renderer, SCREEN_WIDTH, SCREEN_HEIGHT);
	SDL_SetRenderDrawColor(sdl->renderer, 0, 0, 0, 255);
	SDL_SetWindowTitle(sdl->window, "Szablon do zdania drugiego 2017");

	sdl->screen = SDL_CreateRGBSurface(0, SCREEN_WIDTH, SCREEN_HEIGHT, 32,
		0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);

	sdl->scrtex = SDL_CreateTexture(sdl->renderer, SDL_PIXELFORMAT_ARGB8888,
		SDL_TEXTUREACCESS_STREAMING,
		SCREEN_WIDTH, SCREEN_HEIGHT);

	SDL_SetTextureBlendMode(sdl->scrtex, SDL_BLENDMODE_BLEND);

	// wy³¹czenie widocznoœci kursora myszy
	SDL_ShowCursor(SDL_DISABLE);

	return true;
}

// funkcja do ³adowania tekstury która jest animacj¹
SDL_Texture* loadAnimation(SDL_Renderer* ren, const char* filename, int frameWidth, int* outFrames) {
	char fullPath[256];
	snprintf(fullPath, sizeof(fullPath), "%s%s", ANIM_PATH, filename);
	int w, h;
	SDL_Texture* tex = loadTexture(ren, fullPath, &w, &h);
	if (tex) {
		*outFrames = w / frameWidth;
	}
	else {
		*outFrames = 0;
		printf("B³¹d ³adowania: %s\n", filename);
	}
	return tex;
}

// funkcja ³aduj¹ca zasoby (obrazy, czcionki, itp)
bool loadAssets(SDLContext* sdl, GameState* state) {
	// ³adownie czcionki i t³a
	sdl->charset = SDL_LoadBMP("./textures/cs8x8.bmp");
	SDL_SetColorKey(sdl->charset, true, 0x000000);
	sdl->background = loadTexture(sdl->renderer, "./textures/background.bmp", &sdl->bgWidth, &sdl->bgHeight);

	// wczytywanie tekstur gracza
	EntityTextures* pt = &state->player.textures;
	pt->idle = loadAnimation(sdl->renderer, "player_idle.bmp", PLAYER_WIDTH, &pt->idleFrames);
	pt->walk = loadAnimation(sdl->renderer, "player_walk.bmp", PLAYER_WIDTH, &pt->walkFrames);
	pt->attLight = loadAnimation(sdl->renderer, "player_attack_light.bmp", PLAYER_WIDTH, &pt->attLightFrames);
	pt->attHeavy = loadAnimation(sdl->renderer, "player_attack_heavy.bmp", PLAYER_WIDTH, &pt->attHeavyFrames);
	pt->jump = loadAnimation(sdl->renderer, "player_jump.bmp", PLAYER_WIDTH, &pt->jumpFrames);
	pt->dash = loadAnimation(sdl->renderer, "player_dash.bmp", PLAYER_WIDTH, &pt->dashFrames);
	pt->hurt = loadAnimation(sdl->renderer, "player_hurt.bmp", PLAYER_WIDTH, &pt->hurtFrames);
	pt->attShoulder = loadAnimation(sdl->renderer, "player_attack_shoulder.bmp", PLAYER_WIDTH, &pt->attShoulderFrames);

	// wczytywanie tekstur enemy Simple
	EntityTextures* et_1 = &state->enemies[0].textures;
	*et_1 = {};
	et_1->idle = loadAnimation(sdl->renderer, "enemy_simple_idle.bmp", ENEMY_INIT_WIDTH, &et_1->idleFrames);
	et_1->walk = loadAnimation(sdl->renderer, "enemy_simple_walk.bmp", ENEMY_INIT_WIDTH, &et_1->walkFrames);
	et_1->hurt = loadAnimation(sdl->renderer, "enemy_simple_hurt.bmp", ENEMY_INIT_WIDTH, &et_1->hurtFrames);
	et_1->attLight = loadAnimation(sdl->renderer, "enemy_simple_attack_light.bmp", ENEMY_INIT_WIDTH, &et_1->attLightFrames);

	// wczytywanie tekstur enemy Charger
	EntityTextures* et_2 = &state->enemies[1].textures;
	*et_2 = {};
	et_2->idle = loadAnimation(sdl->renderer, "enemy_charger_idle.bmp", ENEMY_INIT_WIDTH, &et_2->idleFrames);
	et_2->walk = loadAnimation(sdl->renderer, "enemy_charger_walk.bmp", ENEMY_INIT_WIDTH, &et_2->walkFrames);
	et_2->hurt = loadAnimation(sdl->renderer, "enemy_charger_hurt.bmp", ENEMY_INIT_WIDTH, &et_2->hurtFrames);
	et_2->dash = loadAnimation(sdl->renderer, "enemy_charger_dash.bmp", ENEMY_INIT_WIDTH, &et_2->dashFrames);

	if (!sdl->charset || !sdl->background || !pt->idle || !pt->walk || !et_1->idle || !et_2->idle) {
		return false;
	}
	return true;
}

// funkcja czyszcz¹ca miejsce po SDL teksturach, renderze, powierzchniach itd
void cleanup(SDLContext* sdl, GameState* state) {
	// Zwalnianie powierzchni (Surface)
	if (sdl->charset) SDL_FreeSurface(sdl->charset);
	if (sdl->screen)  SDL_FreeSurface(sdl->screen);

	// Zwalnianie tekstur renderera i okna
	if (sdl->background)   SDL_DestroyTexture(sdl->background);
	if (sdl->scrtex)   SDL_DestroyTexture(sdl->scrtex);
	if (sdl->renderer) SDL_DestroyRenderer(sdl->renderer);
	if (sdl->window)   SDL_DestroyWindow(sdl->window);

	// Zwalnianie tekstur gracza
	if (state->player.textures.idle) SDL_DestroyTexture(state->player.textures.idle);
	if (state->player.textures.walk) SDL_DestroyTexture(state->player.textures.walk);
	if (state->player.textures.attLight) SDL_DestroyTexture(state->player.textures.attLight);
	if (state->player.textures.attHeavy) SDL_DestroyTexture(state->player.textures.attHeavy);
	if (state->player.textures.jump) SDL_DestroyTexture(state->player.textures.jump);
	if (state->player.textures.hurt) SDL_DestroyTexture(state->player.textures.hurt);

	// Zwalnianie tekstur enemies
	if (state->enemies[0].textures.idle) SDL_DestroyTexture(state->enemies[0].textures.idle);
	if (state->enemies[0].textures.attLight) SDL_DestroyTexture(state->enemies[0].textures.attLight);

	// Wy³¹czenie SDL
	SDL_Quit();
}

//=================================
//       FUNCKCJE DRAW
//=================================

// narysowanie napisu txt na powierzchni screen, zaczynaj¹c od punktu (x, y)
// charset to bitmapa 128x128 zawieraj¹ca znaki
void DrawString(SDL_Surface *screen, int x, int y, const char *text,
                SDL_Surface *charset, int scale, int color) {
	int px, py, c;
	SDL_Rect s, d;
	s.w = DEFAULT_FONT_SIZE;
	s.h = DEFAULT_FONT_SIZE;
	d.w = DEFAULT_FONT_SIZE * scale;
	d.h = DEFAULT_FONT_SIZE * scale;

	Uint8 r, g, b;
	SDL_GetRGB(color, screen->format, &r, &g, &b);
	SDL_SetSurfaceColorMod(charset, r, g, b);

	while(*text) {
		c = *text & 255;
		px = (c % 16) * DEFAULT_FONT_SIZE;
		py = (c / 16) * DEFAULT_FONT_SIZE;
		s.x = px;
		s.y = py;
		d.x = x;
		d.y = y;
		SDL_BlitScaled(charset, &s, screen, &d);
		x += DEFAULT_FONT_SIZE * scale;
		text++;
	};

	SDL_SetSurfaceColorMod(charset, 255, 255, 255);
};

// narysowanie na ekranie screen powierzchni sprite w punkcie (x, y)
// (x, y) to punkt œrodka obrazka sprite na ekranie
void DrawSurface(SDL_Surface *screen, SDL_Surface *sprite, int x, int y) {
	SDL_Rect dest;
	dest.x = x;
	dest.y = y - sprite->h;
	dest.w = sprite->w;
	dest.h = sprite->h;
	SDL_BlitSurface(sprite, NULL, screen, &dest);
	};

// rysowanie pojedynczego pixela
void DrawPixel(SDL_Surface *surface, int x, int y, Uint32 color) {
	// zabezpieczenie do rysowania poza ekranem
	if (x < 0 || x >= surface->w || y < 0 || y >= surface->h) {
		return;
	}
	int bpp = surface->format->BytesPerPixel;
	Uint8 *p = (Uint8 *)surface->pixels + y * surface->pitch + x * bpp;
	*(Uint32 *)p = color;
	};

// rysowanie linii o d³ugoœci l w pionie (gdy dx = 0, dy = 1) 
// b¹dŸ poziomie (gdy dx = 1, dy = 0)
void DrawLine(SDL_Surface *screen, int x, int y, int l, int dx, int dy, Uint32 color) {
	for(int i = 0; i < l; i++) {
		DrawPixel(screen, x, y, color);
		x += dx;
		y += dy;
		};
	};

// rysowanie prostok¹ta o d³ugoœci boków l i k
// gdy fillColor = -1 to bez wnêtrza
void DrawRectangle(SDL_Surface *screen, int x, int y, int l, int k,
                   Uint32 outlineColor, Uint32 fillColor) {
	int i;
	DrawLine(screen, x, y, k, 0, 1, outlineColor);
	DrawLine(screen, x + l - 1, y, k, 0, 1, outlineColor);
	DrawLine(screen, x, y, l, 1, 0, outlineColor);
	DrawLine(screen, x, y + k - 1, l, 1, 0, outlineColor);

	if (fillColor == -1) return;
	for(i = y + 1; i < y + k - 1; i++)
		DrawLine(screen, x + 1, i, l - 2, 1, 0, fillColor);
	};

// funkcja rysuj¹ca pod³ogê i t³o o okreœlonych kolorach
// na podstawie pozycji kamery
void drawScene(SDL_Renderer* renderer, SDL_Texture* background, Camera* camera) {
	SDL_Rect srcRect = {
		(int)camera->x,
		(int)camera->y,
		camera->w,
		camera->h
	};
	SDL_Rect dstRect = { 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT };
	SDL_RenderCopy(renderer, background, &srcRect, &dstRect);


	//SDL_Rect rect = { camera->x, camera->y, camera->w, camera->h };
	//SDL_BlitSurface(background, &dest, screen, NULL);
}

// funkcja rysuj¹ca info na górze ekranu (o czasie gry, fps, punkty, itp)
void drawInfo(SDLContext* sdl, GameState* state) {
	char text[INFO_TEXT_BUFFER_SIZE];

	// pude³ko na info
	DrawRectangle(sdl->screen,
		INFO_MARGIN,
		INFO_MARGIN,
		SCREEN_WIDTH - (2 * INFO_MARGIN),
		INFO_BOX_HEIGHT,
		state->colors.red,
		state->colors.blue
	);

	// linia 1
	sprintf(text, "Czas trwania: %.1lf s  %.0lf fps", state->time.worldTime, state->time.fps);
	DrawString(sdl->screen,
		sdl->screen->w / 2 - (strlen(text) * CHAR_WIDTH) / 2,
		INFO_LINE_1_Y,
		text,
		sdl->charset,
		1,
		state->colors.white
	);

	// linia 2
	sprintf(text, "Zrealizowano: obligatory-5/5 optional-5/8");
	DrawString(sdl->screen,
		sdl->screen->w / 2 - (strlen(text) * CHAR_WIDTH) / 2,
		INFO_LINE_2_Y,
		text,
		sdl->charset,
		1,
		state->colors.white
	);

	sprintf(text, "Uzyskane punkty: %d   Mnoznik combo: %.1lf", state->score.points, state->score.comboMultipler);
	DrawString(sdl->screen,
		sdl->screen->w / 2 - (strlen(text) * CHAR_WIDTH) / 2,
		INFO_LINE_3_Y,
		text,
		sdl->charset,
		1,
		state->colors.white
	);
}

// funkcja frysuj¹ca mno¿nik combo wyœwietlany na dole ekranu
// zwiêksza siê i drga gdy mno¿nik wzrasta
void drawComboMultiplier(SDLContext* sdl, GameState* state) {
	if (state->score.comboMultipler <= 1.0) return;

	char text[32];
	sprintf(text, "COMBO x%.1lf", state->score.comboMultipler);

	// czym wieksze combo tym wiekszy napis
	int scale = 2 + (int)(state->score.comboMultipler)/4;
	if (scale > COMBO_MAX_SCALE) scale = COMBO_MAX_SCALE;

	// ustawienie pozycji
	int textWidth = strlen(text) * DEFAULT_FONT_SIZE * scale;
	int x = (SCREEN_WIDTH / 2) - (textWidth / 2);
	int y = SCREEN_HEIGHT - COMBO_Y_OFFSET;

	// drganie i powiêkszenie
	if (state->score.comboTimer > 1.8) {
		y += (rand() % COMBO_VIBRATION_DEVIATION) - COMBO_DEVIATION_Y_OFFSET;
		x -= COMBO_EFFECT_X_OFFSET;
		scale += 1;
	}

	Uint32 drawColor;
	double combo = state->score.comboMultipler;

	if (combo < COMBO_TRESHOLD_1) drawColor = state->colors.combo1;
	else if (combo < COMBO_TRESHOLD_2) drawColor = state->colors.combo2;
	else if (combo < COMBO_TRESHOLD_3) drawColor = state->colors.combo3;
	else if (combo < COMBO_TRESHOLD_4) drawColor = state->colors.combo4;
	else drawColor = state->colors.combo5;

	DrawString(sdl->screen, x, y, text, sdl->charset, scale, drawColor);
}

// funkcja rysuj¹ca postaæ w odniesieniu do kamery
void drawEntity(SDL_Renderer* renderer, Entity* entity, Camera* camera) {
	SDL_Texture* textureToDraw = getCurrentTexture(entity);

	// wybieranie odpowiedniej klatki
	SDL_Rect srcRect;
	srcRect.x = entity->currentFrame * PLAYER_WIDTH;
	srcRect.y = 0;
	srcRect.w = PLAYER_WIDTH;
	srcRect.h = PLAYER_HEIGHT;

	// wybieranie miejsca na podstawie kamery
	SDL_Rect destRect;
	destRect.x = entity->x - camera->x;
	destRect.y = entity->y - camera->y - entity->w;
	destRect.w = PLAYER_WIDTH;
	destRect.h = PLAYER_HEIGHT;

	// odwracanie w odpowiedni¹ stronê
	SDL_RendererFlip flip = SDL_FLIP_NONE;
	if (entity->direction == LEFT) {
		flip = SDL_FLIP_HORIZONTAL;
	}

	// na czerwono je¿eli hurt
	if (entity->currentAction == ACTION_HURT) SDL_SetTextureColorMod(textureToDraw, 255, 0, 0);
	else SDL_SetTextureColorMod(textureToDraw, 255, 255, 255);

	// render postaci
	SDL_RenderCopyEx(renderer, textureToDraw, &srcRect, &destRect, 0, NULL, flip);
}

// funkcja rysuj¹ca przecinwnika w stosunku do po³o¿enia kamery
void drawEnemies(SDL_Renderer* renderer, GameState* state) {
	Camera* camera = &state->camera;
	for (int i = 0; i < MAX_ENEMIES; i++) {
		Enemy* enemy = &state->enemies[i];
		if (!enemy->active) continue;

		drawEntity(renderer, enemy, camera);
	}
}

// rysowanie na dole ekranu bufora i aktualnej akcji (tryb debug)
void drawDebugOverlay(SDLContext* sdl, GameState* state) {
	if (state->buffer.showDebug) {
		char text[512];
		char temp[64];

		sprintf(text, "DEBUG: Action: %s | Buffer: [ ", actionNames[state->player.currentAction]);
		for (int i = 0; i < INPUT_BUFFER_SIZE; i++) {
            sprintf(temp, "%s ", inputNames[state->buffer.inputs[i].input]);
            strcat(text, temp);
        }
        strcat(text, "]");

		DrawString(sdl->screen, 10, SCREEN_HEIGHT-15, text, sdl->charset, 1, state->colors.white);
	}
}

// funkcja rusuj¹ca prostok¹ty pokazuj¹ce hitboxy i hurtboxy wszystkich postaci
void drawHitboxes(SDLContext* sdl, GameState* state) {
	if (!state->buffer.showDebug) return;

	Camera* cam = &state->camera;
	Colors* col = &state->colors;

	// rysowanie hurtboxa playera
	SDL_Rect playerBox = getHurtbox(&state->player);
	int px = playerBox.x - cam->x;
	int py = playerBox.y - cam->y;
	DrawRectangle(sdl->screen, px, py, playerBox.w, playerBox.h, col->green, -1);

	// Rysowanie hitboxa ataków playera
	SDL_Rect attackBox = getHitBox(&state->player);
	if (attackBox.w > 0) {
		DrawRectangle(sdl->screen,
			attackBox.x - cam->x,
			attackBox.y - cam->y,
			attackBox.w,
			attackBox.h,
			col->red, -1);
	}

	for(int i = 0; i < MAX_ENEMIES; i++) {
		Enemy* enemy = &state->enemies[i];
		if (!enemy->active) continue;

		// Rysowanie hurtboxa enemy
		SDL_Rect enemyBox = getHurtbox(enemy);
		int ex = enemyBox.x - cam->x;
		int ey = enemyBox.y - cam->y;
		DrawRectangle(sdl->screen, ex, ey, enemyBox.w, enemyBox.h, col->green, -1);

		// Rysowanie hitboxa ataków enemy
		SDL_Rect enemyAttackBox = getHitBox(enemy);
		if (enemyAttackBox.w > 0) {
			DrawRectangle(sdl->screen,
				enemyAttackBox.x - cam->x,
				enemyAttackBox.y - cam->y,
				enemyAttackBox.w,
				enemyAttackBox.h,
				col->red, -1);
		}
	}
	
}

//=================================
//       FUNCKCJE LOGIC
//=================================

// funkcja blokuj¹ca mo¿liwoœæ wyjœcia poza obszar gry danej postaci
void keepEntityInGameArea(Entity* entity, int mapWidth) {
	// z góry
	if (entity->y < HORIZON_Y) {
		entity->y = HORIZON_Y;
	}
	// z do³u
	if (entity->y > SCREEN_HEIGHT) {
		entity->y = SCREEN_HEIGHT;
	}
	// z lewej
	if (entity->x < 0 - HURTBOX_OFF_X) {
		entity->x = 0 - HURTBOX_OFF_X;
	}
	// z prawej
	if (entity->x > mapWidth - entity->w + HURTBOX_OFF_X) {
		entity->x = mapWidth - entity->w + HURTBOX_OFF_X;
	}
}

// funkcja wywo³ywana gdy postaæ rozpoczyna dan¹ akcjê o danym czasie trwania
// (zerowanie timerów, frame counterów, itp)
void startAction(Entity* entity, int action) {
	entity->currentAction = action;
	entity->hasHit = false;
	entity->currentFrame = 0;
	entity->animTimer = 0.0;
	entity->actionTimer = getActionDuration(action);
}

// funkcja obs³uguj¹ca kolizje hitboxów ataków i hurtboxów postaci
// gracz uderzea wroga / wróg uderza gracza
// dawna handleAttack
void handleDamage(GameState* state) {
	SDL_Rect playerAttackBox = getHitBox(&state->player);
	SDL_Rect playerHurtBox = getHurtbox(&state->player);

	for (int i = 0; i < MAX_ENEMIES; i++) {
		Enemy* enemy = &state->enemies[i];
		if (!enemy->active) continue;

		SDL_Rect enemyAttackBox = getHitBox(enemy);
		SDL_Rect enemyHurtBox = getHurtbox(enemy);

		// czy player uderzy³ enemy
		if (checkRectCollision(playerAttackBox, enemyHurtBox) && !state->player.hasHit) {
			// po uderzeniu stun i reset akcji przeciwnika
			state->player.hasHit = true;
			enemy->hitTimer = TIME_HURT;
			enemy->aiState = AI_STATE_ALIGN;
			enemy->aiTimer = 0.0;
			enemy->attackCooldown = TIME_HURT;

			// aktualizacja combo i punktów
			state->score.comboTimer = COMBO_COOLDOWN;
			state->score.points += attacksData[state->player.currentAction].damage * state->score.comboMultipler;
			state->score.comboMultipler += COMBO_MULTIPLER_ADD_VALUE;

			// przeciwnik obrywa
			startAction(enemy, ACTION_HURT);

			if (state->buffer.showDebug) printf("HIT! Action ID: %d\n", state->player.currentAction);
		}

		// czy enemy uderzy³ playera
		if (checkRectCollision(enemyAttackBox, playerHurtBox) && !enemy->hasHit) {
			enemy->hasHit = true;

			//resetujê licznik combo
			state->score.comboMultipler = 0.0;
			state->score.comboTimer = 0.0;

			startAction(&state->player, ACTION_HURT);
			if (state->buffer.showDebug) {
				printf("PLAYER HIT BY ENEMY! Action ID: %d\n", enemy->currentAction);
			}
		}
	}
}

// funkcja sprawdzaj¹ca czy w buforze znajduje siê dana sekwencja
// (na ostatnim miejscu nie wczeœniej)
bool checkSequence(Buffer* buffer, Sequence* sequence) {
	for (int i = 0; i < sequence->len; i++) {
		int requiredInput = sequence->sequence[sequence->len - 1 - i];
		InputEvent* event = getInputBack(buffer, i);

		// sprawdzanie czy odpowiedni input
		if (event->input != requiredInput) {
			return false;
		}

		// sprawdzanie czy za wolno
		if (i > 0) {
			InputEvent* nextEvent = getInputBack(buffer, i - 1);
			Uint32 diff = nextEvent->time - event->time;
			if (diff > sequence->maxTimeGap) {
				return false;
			}
		}
	}

	// Jeœli pêtla przesz³a do koñca, to znaczy, ¿e wszystko pasuje!
	return true;
}

// funkcja wykonuj¹ca akcjê w odniesieniu do wprowadzonych inputów
void resolveInputs(GameState* state) {
	// je¿eli jest uderzony to nie mo¿e wykonywaæ akcji (stun)
	if (state->player.currentAction == ACTION_HURT
		|| state->player.currentAction == ACTION_SHOULDER
		|| state->player.currentAction == ACTION_JUMP
		|| state->player.currentAction == ACTION_DASH) return;

	// sprawdzanie combosów
	for (int i = 0; i < state->sequencesCount; i++) {
		if (checkSequence(&state->buffer, &state->definedSequences[i])) {

			// wyswietlanie w consoli dla opcji debug
			if (state->buffer.showDebug) {
				printf(">>> COMBO DETECTED: %s <<<\n", state->definedSequences[i].name);
			}

			// co ma sie staæ w danym combo
			if (strcmp(state->definedSequences[i].name, "Shoulder") == 0) {
				startAction(&state->player, ACTION_SHOULDER);
			}
			else if (strcmp(state->definedSequences[i].name, "Jump") == 0) {
				startAction(&state->player, ACTION_JUMP);
			}
			else if (strcmp(state->definedSequences[i].name, "Dash Right") == 0) {
				startAction(&state->player, ACTION_DASH);
				state->player.direction = RIGHT;
			}
			else if (strcmp(state->definedSequences[i].name, "Dash Left") == 0) {
				startAction(&state->player, ACTION_DASH);
				state->player.direction = LEFT;
			}
			return;
		}
	}

	// sprawdzanie podjedyñczych klikniêæ tylko w idle i walk
	if (state->player.currentAction == ACTION_IDLE || 
		state->player.currentAction == ACTION_WALK) {
		InputEvent* lastEvent = getInputBack(&state->buffer, 0);

		if (lastEvent->input == INPUT_ATTACK_LIGHT) {
			startAction(&state->player, ACTION_LIGHT);
		}

		if (lastEvent->input == INPUT_ATTACK_HEAVY) {
			startAction(&state->player, ACTION_HEAVY);
		}
	}
}

// funkcja aktualizuj¹ca zmienne odnoœnie punktów i combo
void updateScoreLogic(Score* score, double delta) {
	// ograniczenie combo
	if (score->comboMultipler > MAX_MULTIPLER_VALUE) {
		score->comboMultipler = MAX_MULTIPLER_VALUE;
	}

	if(score->comboTimer > 0) {
		score->comboTimer -= delta;
	} else {
		score->comboMultipler = 1.0;
	}
}

// funkcja przesuwaj¹ca obecn¹ klatkê danej postaci na podstawie d³ugoœci animacji/klatki oraz iloœci klatek
void handleFramesAnimation(Entity* entity, int maxFrames, double durationOrSpeed, double delta) {
	if (maxFrames <= 0) return;

	// dla idle i walk prêdkoœæ animacji a dla ataków czas trwania ca³oœci
	double frameDuration;
	if (entity->currentAction == ACTION_IDLE || entity->currentAction == ACTION_WALK) {
		frameDuration = durationOrSpeed;
	}
	else {
		frameDuration = durationOrSpeed / (double)maxFrames;
	}

	entity->animTimer += delta;
	// przesuwanie klatki gdy jej czas min¹³
	if (entity->animTimer >= frameDuration) {
		entity->animTimer -= frameDuration;
		entity->currentFrame++;

		if (entity->currentFrame >= maxFrames) {
			entity->currentFrame = 0;
		}
	}
}

// funkcja aktualizuj¹ca animacjê danej postaci (w zale¿noœci od akcji wykonywanej)
void updateEntityAnimation(Entity* entity, double delta) {
	int maxFrames = getActionFrames(entity);
	double duration = getActionDuration(entity->currentAction);

	handleFramesAnimation(entity, maxFrames, duration, delta);
}

// funkcja aktualizuj¹ca akcjê gracza w zale¿noœci od czasu i bufora
void updatePlayerAction(GameState* state) {
	if (state->buffer.headIndex != state->buffer.previousHeadIndex) {
		// sprawdzanie któr¹ akcjê ma wykonaæ i wykonywanie
		resolveInputs(state);
		state->buffer.previousHeadIndex = state->buffer.headIndex;
	}

	// je¿eli jest w akcji to p³ynie czas akcji
	if (state->player.currentAction != ACTION_IDLE && state->player.currentAction != ACTION_WALK) {
		state->player.actionTimer -= state->time.delta;

		// Jeœli czas min¹³ koniec ataku
		if (state->player.actionTimer <= 0) {
			state->player.currentAction = ACTION_IDLE;
			state->player.actionTimer = 0;
		}
	}
}

// funkcja obs³uguj¹ca ruch gracza. Odbiera klikniêcia i przytrzymania 
// klawiszy WSAD i zmienia pozycjê x,y gracza
void movePlayer(Player* player, double delta, int bgWidth) {
	// je¿eli jest uderzony to ma stun (nie mo¿e siê ruszaæ)
	if (player->currentAction == ACTION_HURT) return;

	// obs³uga dasha
	if (player->currentAction == ACTION_DASH) {
		if (player->direction == RIGHT) {
			player->x += (DASH_SPEED * delta);
		}
		else {
			player->x -= (DASH_SPEED * delta);
		}
		return;
	}

	// je¿eli player jest w akcji to nie mo¿e siê ruszaæ
	if (player->currentAction == ACTION_LIGHT ||
		player->currentAction == ACTION_HEAVY ||
		player->currentAction == ACTION_SHOULDER ||
		player->currentAction == ACTION_JUMP) {
		return;
	}
	
	// domyœlnie stan idle
	player->currentAction = ACTION_IDLE;

	// obs³uga klawiszy WSAD
	bool isMoving = false;
	const Uint8* keystate = SDL_GetKeyboardState(NULL);

	if(keystate[SDL_SCANCODE_W]){
		player->y -= (player->speed * delta);
		isMoving = true;
	}
	if(keystate[SDL_SCANCODE_S]){
		player->y += (player->speed * delta);
		isMoving = true;
	}
	if(keystate[SDL_SCANCODE_A]){
		player->x -= (player->speed * delta);
		player->direction = LEFT;
		isMoving = true;
	}
	if(keystate[SDL_SCANCODE_D]){ 
		player->x += (player->speed * delta);
		player->direction = RIGHT;
		isMoving = true;
	}
	if (isMoving) player->currentAction = ACTION_WALK;

	keepEntityInGameArea(player, bgWidth);
}

// funkcja g³ówna obs³uguj¹ca zmiany playera
void updatePlayer(GameState* state, int bgWidth) {
	updatePlayerAction(state);
	movePlayer(&state->player, state->time.delta, bgWidth);
	updateEntityAnimation(&state->player, state->time.delta);
}

// aktualizacja czasu i fps
void updateTime(Time *time) {
	time->t2 = SDL_GetTicks();
	time->delta = (time->t2 - time->t1) * 0.001;
	time->t1 = time->t2;
	time->worldTime += time->delta;

	// obliczanie FPS
	time->fpsTimer += time->delta;
	if (time->fpsTimer > FPS_REFRESH) {
		time->fps = time->frames * 2;
		time->frames = 0;
		time->fpsTimer -= FPS_REFRESH;
	};
	time->frames++;
}

// funkcja aktualizuj¹ca pozycjê kamery, przesuwa kamerê je¿eli
// gracz napiera na któr¹œ z œciañ ekranu (z marginesem)
void updateCamera(GameState* state, int mapWidth) {
	// gdy gracz jest przy prawej krawêdzi ekranu to ruch kamer¹
	if ((state->player.x + state->player.w) > state->camera.x + SCREEN_WIDTH - CAMERA_MARGIN) {
		state->camera.x = (state->player.x + state->player.w) - (SCREEN_WIDTH - CAMERA_MARGIN);
	}// gdy gracz jest przy lewej krawêdzi ekranu to ruch kamer¹
	if (state->player.x < state->camera.x + CAMERA_MARGIN) {
		state->camera.x = state->player.x - CAMERA_MARGIN;
	}

	// blokada po lewej
	if (state->camera.x < 0) {
		state->camera.x = 0;
	}// blokada po prawej
	if (state->camera.x > mapWidth - SCREEN_WIDTH) {
		state->camera.x = mapWidth - SCREEN_WIDTH;
	}
}

// funkcja wrzucaj¹ca dany input do bufora, je¿êli ma wpisaæ poza
// to nadpisuje pocz¹tek
void pushInput(Buffer* buffer, int inputCode) {
	buffer->inputs[buffer->headIndex].input = inputCode;
	buffer->inputs[buffer->headIndex].time = SDL_GetTicks();
	buffer->headIndex = (buffer->headIndex + 1) % INPUT_BUFFER_SIZE;
}

// funkcja sprawdzaj¹ca czy player znajduje siê w obrêbie ataku lub dasha przeciwnika
bool canAttackEnemy(Enemy* enemy, Player* player) {
	if (enemy->attackCooldown > 0) return false;

	// obliczanie odleg³oœci
	double distX = player->x - enemy->x;
	double distY = player->y - enemy->y;

	if (enemy->type == ENEMY_TYPE_SIMPLE) {
		// czy jest w zasiêgu
		return (fabs(distX) < ENEMY_SIMPLE_ATTACK_RANGE && fabs(distY) < ENEMY_SIMPLE_ATTACK_RANGE / 2);
	}
	else if (enemy->type == ENEMY_TYPE_CHARGER) {
		// czy jest w tej samej lini
		return (fabs(distY) < CHARGER_ALIGN_Y_MARGIN);
	}

	return false;
}

// funkcja poruszaj¹ca przeciwnika w stronê gracza w najprostrzy sposób
// przecinik typ SIMPLE
void moveEnemySimple(Enemy* enemy, Player* player, double delta) {
	double distX = player->x - enemy->x;
	double distY = player->y - enemy->y;

	// ustawianie kierunku (patrzy na gracza)
	if (distX > 0) enemy->direction = RIGHT;
	else enemy->direction = LEFT;

	// ruch w stronê gracza w osi x
	if (fabs(distX) > ENEMY_SIMPLE_ATTACK_RANGE - 15) {
		if (distX > 0) enemy->x += ENEMY_SPEED_WALK * delta;
		else enemy->x -= ENEMY_SPEED_WALK * delta;
		enemy->currentAction = ACTION_WALK;
	}

	// ruch w stronê gracza w osi y
	if (fabs(distY) > 10) {
		if (distY > 0) enemy->y += (ENEMY_SPEED_WALK * 0.6) * delta;
		else enemy->y -= (ENEMY_SPEED_WALK * 0.6) * delta;
		enemy->currentAction = ACTION_WALK;
	}
}

// funkcja obs³uguj¹ca zachowanie przeciwnika
// przeciwnik typ SIMPLE
void updateSimpleEnemyBehavior(Enemy* enemy, Player* player, double delta) {
	// obs³uga czy w ataku
	if (enemy->currentAction == ACTION_LIGHT) {
		enemy->actionTimer -= delta;

		if (enemy->actionTimer <= 0) {
			enemy->currentAction = ACTION_IDLE;
			enemy->actionTimer = 0;
		}
		return;
	}

	// przygotowanie do ataku
	if (enemy->aiState == AI_STATE_PREPARE) {
		enemy->aiTimer -= delta;
		enemy->currentAction = ACTION_IDLE;

		// jak czas min¹³ to atak
		if (enemy->aiTimer <= 0) {
			startAction(enemy, ACTION_LIGHT);
			enemy->attackCooldown = ENEMY_ATTACK_COOLDOWN;
			enemy->aiState = 0;
		}
		return;
	}

	// wybiera czy zaatakowaæ czy podejœæ
	if (canAttackEnemy(enemy, player)) {
		// przejœcie do przygotowania do ataku
		enemy->aiState = AI_STATE_PREPARE;
		enemy->aiTimer = ENEMY_SIMPLE_PREPARE_TIME;
	}
	else {
		moveEnemySimple(enemy, player, delta);
	}
}

// funkcja poruszaj¹ca przeciwnikiem aby wyrówna³ siê z y gracza
// przecinwik typ CHARGER
void alignEnemyCharger(Enemy* enemy, Player* player, double delta) {
	double distX = player->x - enemy->x;
	double distY = player->y - enemy->y;

	// odwracanie siê w kierunku playera
	if (distX > 0) enemy->direction = RIGHT;
	else enemy->direction = LEFT;

	double targetX = (player->x > enemy->x) ? (player->x - CHARGER_KEEP_DIST) : (player->x + CHARGER_KEEP_DIST);

	// ruch w kierunku x - oddalony od playera o dist, y - playera
	bool moving = false;
	if (fabs(enemy->x - targetX) > 10) {
		if (enemy->x < targetX) enemy->x += ENEMY_SPEED_WALK * delta;
		else enemy->x -= ENEMY_SPEED_WALK * delta;
		moving = true;
	}
	if (fabs(distY) > CHARGER_ALIGN_Y_MARGIN) {
		if (distY > 0) enemy->y += ENEMY_SPEED_WALK * delta;
		else enemy->y -= ENEMY_SPEED_WALK * delta;
		moving = true;
	}

	// ustawianie odpowiedniej akcji
	if (moving) enemy->currentAction = ACTION_WALK;
	else enemy->currentAction = ACTION_IDLE;

	// przechodzi w stan przygotowywania siê
	if (canAttackEnemy(enemy, player)) {
		enemy->aiState = AI_STATE_PREPARE;
		enemy->aiTimer = TIME_CHARGER_PREPARE;
		enemy->currentAction = ACTION_IDLE;
	}
}

// funkcja oczekuj¹ca na szar¿ê (odliczanie czasu)
// przeciwnik typ CHARGER
void prepareEnemyCharger(Enemy* enemy, double delta) {
	// odliczanie do ataku
	enemy->aiTimer -= delta;

	// atak
	if (enemy->aiTimer <= 0) {
		enemy->aiState = AI_STATE_CHARGE;
		enemy->aiTimer = TIME_CHARGE;
		startAction(enemy, ACTION_CHARGE);
	}
}

// funkcja wykonuj¹ca szar¿ê we wczeœniej obranym kierunku
// przeciwnik typ CHARGER
void doEnemyCharge(Enemy* enemy, double delta) {
	enemy->aiTimer -= delta;

	// wykonanie szar¿y w danym kierunku
	if (enemy->direction == RIGHT) enemy->x += ENEMY_SPEED_CHARGE * delta;
	else enemy->x -= ENEMY_SPEED_CHARGE * delta;

	// gdy koniec powrót do idle oraz na³o¿enie cooldownu
	if (enemy->aiTimer <= 0) {
		enemy->aiState = AI_STATE_COOLDOWN;
		enemy->attackCooldown = ENEMY_ATTACK_COOLDOWN;
		startAction(enemy, ACTION_IDLE);
	}
}

// funkcja obs³uguj¹ca zachowanie przeciwnika
// przeciwnik typ CHARGER
void updateChargerEnemyBehavior(Enemy* enemy, Player* player, double delta) {
	switch (enemy->aiState) {
		case AI_STATE_ALIGN:
			alignEnemyCharger(enemy, player, delta);
			break;
		case AI_STATE_PREPARE:
			prepareEnemyCharger(enemy, delta);
			break;
		case AI_STATE_CHARGE:
			doEnemyCharge(enemy, delta);
			break;
		case AI_STATE_COOLDOWN:
			// jak siê skoñczy cooldown to po prostu zmiana na ALIGN
			enemy->aiState = AI_STATE_ALIGN;
			break;
	}
}

// funkcja logiki zachowania przeciwników
void updateEnemies(GameState* state, double delta, int bgWidth) {
	Player* player = &state->player;
	for (int i = 0; i < MAX_ENEMIES; i++) {
		Enemy* enemy = &state->enemies[i];
		// sprawdzanie czy istnieje
		if (!enemy->active) continue;

		// obs³uga stuna
		if (enemy->hitTimer > 0) {
			enemy->hitTimer -= delta;
			updateEntityAnimation(enemy, delta);
			continue;
		}

		// obs³uga cooldownu
		if (enemy->attackCooldown > 0) {
			enemy->attackCooldown -= delta;
		}

		// wybór zachowania odnoœnie typu przeciwnika
		switch (enemy->type) {
			case ENEMY_TYPE_SIMPLE:
				updateSimpleEnemyBehavior(enemy, player, delta);
				break;
			case ENEMY_TYPE_CHARGER:
				updateChargerEnemyBehavior(enemy, player, delta);
				break;
		}

		keepEntityInGameArea(enemy, bgWidth);
		updateEntityAnimation(enemy, delta);
	}
}

//=================================
//   G£ÓWNE FUNKCJE LOGIKI GRY
//=================================

// obs³ugiwanie zdarzeñ klikniêæ z klawiatury oraz zamkniêcie okna
void handleEvents(GameState* state, const SDLContext* sdl) {
	SDL_Event event;
	while (SDL_PollEvent(&event)) {
		switch (event.type) {
			case SDL_KEYDOWN:
				if (event.key.keysym.sym == SDLK_ESCAPE) state->quit = 1;
				if (event.key.keysym.sym == SDLK_n) initGameState(state, sdl);

				if (event.key.repeat == 0) {
					int gameInput = INPUT_NONE;
					switch (event.key.keysym.sym) {
						case SDLK_w:	gameInput = INPUT_UP; break;
						case SDLK_s:	gameInput = INPUT_DOWN; break;
						case SDLK_a:	gameInput = INPUT_LEFT; break;
						case SDLK_d:    gameInput = INPUT_RIGHT; break;
						case SDLK_p:	gameInput = INPUT_ATTACK_LIGHT; break;
						case SDLK_o:	gameInput = INPUT_ATTACK_HEAVY; break;
						case SDLK_t:	state->buffer.showDebug = !state->buffer.showDebug; break;
					}
					if (gameInput != INPUT_NONE) {
						pushInput(&state->buffer, gameInput);
					}
				}
				break;
			case SDL_QUIT:
				state->quit = 1;
				break;
			};
	};
}

// aktualizacja stanu gry
void updateGame(GameState* state, SDLContext* sdl) {
	updateTime(&state->time);
	updateEnemies(state, state->time.delta, sdl->bgWidth);
	updatePlayer(state, sdl->bgWidth);
	updateCamera(state, sdl->bgWidth);
	handleDamage(state);
	updateScoreLogic(&state->score, state->time.delta);
}

// renderowanie klatki
void render(GameState* state, SDLContext* sdl) {
	// czyszczenie ekranu przed now¹ klatk¹
	SDL_FillRect(sdl->screen, NULL, SDL_MapRGBA(sdl->screen->format, 0, 0, 0, 0));

	drawScene(sdl->renderer, sdl->background, &state->camera);
	drawEnemies(sdl->renderer, state);

	// rysowanie gracza
	drawEntity(sdl->renderer, &state->player, &state->camera);

	drawHitboxes(sdl, state);
	drawInfo(sdl, state);
	drawDebugOverlay(sdl, state);
	drawComboMultiplier(sdl, state);

	// wysy³anie na ekran
	SDL_UpdateTexture(sdl->scrtex, NULL, sdl->screen->pixels, sdl->screen->pitch);
	SDL_RenderCopy(sdl->renderer, sdl->scrtex, NULL, NULL);
	SDL_RenderPresent(sdl->renderer);
}

//=================================
//         FUNKCJA MAIN
//=================================

#ifdef __cplusplus
extern "C"
#endif
int main(int argc, char **argv) {
	SDLContext sdl = {};
	GameState state;

	// inicjalizacja biblioteki SDL
	if (!initSDL(&sdl)) {
		cleanup(&sdl, &state);
		return 1;
	}

	// pobieranie zasobów
	if (!loadAssets(&sdl, &state)) {
		cleanup(&sdl, &state);
		return 1;
	}

	// inicjalizacja stanu gry
	initGameState(&state, &sdl);

	// g³ówna pêtla gry
	while(!state.quit) {
		handleEvents(&state, &sdl);
		updateGame(&state, &sdl);
		render(&state, &sdl);
	};

	cleanup(&sdl, &state);
	return 0;
};
